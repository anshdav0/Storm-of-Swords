...
import { getVillage, addBuilding, saveLayout, upgradeBuilding, collectResources } from "../api";
...
export function useVillageLayout() {
    ...
    const [placingBuilding, setPlacingBuilding] = useState<{
        buildingId: number; sizeX: number; sizeY: number;
    } | null>(null);
    ...
    const buyBuildingMutation = useMutation({
        mutationFn: ({ buildingId, x, y, placements }: {
            buildingId: number; x: number; y: number; placements: BuildPlacement[];
    ...
    const upgradeMutation = useMutation({
        mutationFn: (villageBuildingId: number) => upgradeBuilding(villageBuildingId),
    ...
    const collectMutation = useMutation({
        mutationFn: (villageBuildingId: number) => {
            ...
            const resourceType = (b?.resource_type ?? "gold") as "gold" | "iron" | "wildfire";
